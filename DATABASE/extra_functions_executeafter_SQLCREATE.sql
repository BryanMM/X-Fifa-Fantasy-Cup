...
as begin
	...
	if (select ASYMKEY_ID(@user_username))>=0
		begin
			...
			begin
				select axi.user_type_id as user_type,axi.adminxinfo_id as user_login, ad.admin_active as user_active from admin as ad left outer join adminxinfo as axi on (ad.admin_username = axi.admin_username) where ad.admin_username = @user_username;
				set @result = 1;
...
as begin
	...
	declare @t_1 int;
	declare @t_2 int;
	declare @table table(winner_1 int,winner_2 int,match_id int,team_1 int,team_2 int,name_team_1 varchar(255),name_team_2 varchar(255));
